...
namespace MVVMExpenses.Common.ViewModel {
    ...
    public abstract class SingleObjectViewModelBase<TEntity, TPrimaryKey, TUnitOfWork> : ISingleObjectViewModel<TEntity, TPrimaryKey>, ISupportParameter, IDocumentContent, ISupportLogicalLayout<TPrimaryKey>
        ...
        public void ResetLayout() {
            MessageResult confirmationResult = MessageBoxService.ShowMessage(CommonResources.Confirmation_ResetLayout, CommonResources.Confirmation_Caption, MessageButton.OKCancel);
            if(confirmationResult == MessageResult.OK)
                ViewModelLogicalLayoutHelper.PersistentViewsLayout.Remove(ViewName);
        }
        ...
        protected virtual string GetTitleForNewEntity() {
            return "New " + typeof(TEntity).Name;
        }
		...
		IDocumentManagerService ISupportLogicalLayout.DocumentManagerService {
            get { return ((ISupportLogicalLayout)((ISupportParentViewModel)this).ParentViewModel).DocumentManagerService; }
        }

		IEnumerable<object> ISupportLogicalLayout.LookupViewModels {
            get { return lookUpViewModels.Select(vm => vm.Value).OfType<ISupportLogicalLayout>(); }
        }
        #endregion
    }
}